...
/**
 * VISÃO GERAL — o estado de repouso do trilho, quando nenhuma nota está aberta. Uma
 * síntese editorial da revisão: quantas anotações, a divisão entre o que a ferramenta
 * resolve e o que fica com o autor, filtros por critério e as métricas de apoio —
 * compactas, subordinadas ao texto, sem dashboard nem gráficos de rosca.
 */
  ...
  return (
    <div className="fade-in flex flex-col">
      ...
      <section className="px-6 pb-6 pt-6">
        ...
        {total > 0 && (
          <>
            <div className="mt-5 flex h-2 overflow-hidden rounded-full bg-surface-3" role="img" aria-label={`${safeCount} seguras, ${humanCount} exigem decisão humana`}>
                ...
                {(["error", "warning", "info"] as Severity[]).map((s) =>
                  sev[s] > 0 ? (
                    <span key={s} className="inline-flex items-center gap-1.5">
                      <span className="size-[6px] rounded-full" style={{ background: severityInkVar(s) }} aria-hidden />
            ...
            return (
              <li key={criterion}>
                ...
                >
                  ...
                  <span className="w-5 text-right text-[13px] tabular-nums text-ink-1">{count}</span>
                </button>
              </li>
            );
          })}
      ...
      <section className="border-t border-rule-1 px-6 py-5">
        ...
        <dl className="mt-1 flex flex-col">
          {metricRows(diagnostic).map((r) => (
            <div key={r.label} className="flex items-baseline justify-between border-b border-rule-1 py-2 last:border-0">
