...
Um instrumento que **diagnostica** um texto contra os critérios de Linguagem Simples, **pontua** cada critério, **aponta** cada trecho ao princípio que ele viola, e **propõe** as edições que dá pra fazer com segurança mecânica — cada apontamento carimbado com o critério que disparou.
...
3. **Nominalização** (`5.3.3`/`5.3.4`) — verbo pesado escondido em substantivo (`-ção`, `-mento`, `-ância` + verbo leve: "fazer a análise de" → "analisar"). Sugestão mecânica só quando o mapeamento é único.
4. **Jargão / termo incomum** (`5.3.2` — palavras familiares) — cruza contra (a) léxico de termos técnicos do domínio e (b) lista de frequência do PT-BR. Substituição só quando o glossário tem equivalente **único e seguro**; sentido múltiplo ("banco", "manga") → sinaliza, não troca.
...
type Finding = {
  ...
  suggestion?: string;       // só quando mecanicamente segura
...
- Sugestões mecânicas vêm carimbadas com o critério; `requiresHuman: true` vira um cartão separado "exige decisão humana" (reorganização pro leitor, corte do supérfluo, agente omitido) — a ferramenta **não** resolve, e diz por quê.
...
- ❌ Deixar o LLM reescrever, sugerir reescrita ou "melhorar" texto. Ele só lê e reporta.
