...
1. O LLM **da sonda** nunca reescreve, sugere reescrita ou "melhora" texto. Só lê e reporta. (A reescrita por IA do Tier 3 — ADR-019 — é caminho à parte: existe como PROPOSTA, sempre verificada pela engine e aplicada só por decisão do autor; nunca é a sonda.)
...
2. **Voz passiva** (`5.3.3` — frases claras, quem faz o quê) — detecção morfossintática (`ser` + particípio; `estar` ficou **permanentemente fora** do pass — ADR-052). Flag; se o agente foi omitido, marcar `requiresHuman` (não dá pra ativar sem inventar quem agiu).
...
4. **Jargão / termo incomum** (`5.3.2` — palavras familiares) — cruza contra (a) léxico de termos técnicos do domínio e (b) lista de frequência do PT-BR. **Nota de estado:** só (a) foi ligado; a perna (b) por frequência **nunca entrou em runtime** — o ADR-008 relegou raridade-por-frequência à curadoria offline (o knob `frequencyRankCutoff` fica declarado, sem consumidor). Equivalente do glossário exibido como **informação** só quando único e seguro — a engine indica, nunca troca; sentido múltiplo ("banco", "manga") → sinaliza, sem equivalente.
5. **Legibilidade** (`5.4` — sinal de apoio, não selo) — usar o **Flesch adaptado ao PT-BR de Martins et al. (1996)**, não o Flesch do inglês (**implementado**). Para as demais métricas de complexidade, a intenção original era reusar o **NILC-Metrix** (nilc-nlp/nilcmetrix, ~200 métricas, linhagem PorSimples/USP) em vez de recalcular pesos — **aspiração ainda não implementada** (hoje só o Flesch-PT está no código). Reportar antes/depois pra provar que ficou mais simples, não só diferente. **Sempre subordinar a métrica aos Princípios 2–3: leiturabilidade boa não é aprovação.**
...
- **NILC-Metrix** (nilc-nlp/nilcmetrix, GitHub) — ~200 métricas de complexidade/coesão, linhagem PorSimples/USP (grupo Sandra Aluísio). Reuso **pretendido** para o placar métrico — **ainda não integrado** (hoje o placar usa só o Flesch-PT).
