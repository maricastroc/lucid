...
describe.runIf(process.env.AB_PLAN === "1")("plano (offline, zero chamadas)", () => {
  it("mostra os alvos, o custo estimado e o teto antes de qualquer chamada", () => {
    ...
    say(`\n=== PLANO ETAPA 1 ===`);
    say(`alvos: ${targets.length} · candidatos: ${CANDIDATES.length} · modelos: ${MODELS.length} (${MODELS.join(", ")})`);
    ...
    say(`critérios no briefing: ${[...criteria.entries()].sort((a, b) => b[1] - a[1]).map(([c, n]) => `${c}=${n}`).join(", ")}`);
...
describe.runIf(process.env.AB === "1")("etapa 1 — evolução do prompt (rede)", () => {
  it(
    "roda os quatro candidatos nos mesmos alvos, modelos e parâmetros",
    async () => {
      const targets = loadEvalTargets(TARGET_COUNT);
      ...
      say(`\n=== ETAPA 1 ===`);
      say(`alvos: ${targets.length} · candidatos: ${candidates.map((c) => c.id).join(", ")} · modelos: ${MODELS.join(", ")}`);
      say(`plano: ${jobs.length} chamadas · teto: ${STAGE1_BUDGET.maxCalls} chamadas / ${STAGE1_BUDGET.maxTokens} tokens`);
...
describe.runIf(process.env.AB_STAGE2 === "1")("etapa 2 — redução de contexto (rede)", () => {
  it(
    "roda o prompt escolhido com documento inteiro × janelas de ±1, ±2 e ±3 parágrafos",
    async () => {
...
describe.runIf(process.env.AB_REPORT === "1")("relatório (offline, zero chamadas)", () => {
  it(
    "pontua as respostas salvas e escreve as tabelas e a amostra cega",
    async () => {
      ...
      say(`\n${SCOPE}\n${tables}\n`);
      say(`amostra cega: ${sample.relevant} blocos com divergência relevante, ${sample.cosmetic} só de redação, ${sample.identical} idênticos\n`);
    },
    900_000,
  );
});
...
describe.runIf(process.env.AB_FINAL === "1")("decisão final entre as duas finalistas (offline)", () => {
  it(
    "abre o veto, refaz as métricas sobre os 20 pares completos e monta a amostra cega reduzida",
    async () => {
      const model = process.env.AB_MODEL ?? "gemini-2.5-flash";
